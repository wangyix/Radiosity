...
// constants
uniform uint _id;				// ID of receiver quad
uniform vec3 _reflectance;		// color of receiver quad

uniform usampler2D _visTex;		// texture of IDs from visibility pass

uniform vec3 _shooterPower;		// (residual) shooter irradiance * shooter area
uniform vec3 _normal;			// normal of receiver in shooter viewspace

uniform sampler2D _radTex;		// radiosity texture of receiver
uniform sampler2D _resTex;		// residual texture of receiver


in vec3 vPosition;		// in shooter viewspace
in vec2 vTexcoord;
...
void main(void) {
	...
	uint visId = texture2D(_visTex, visTexcoord).x;
	bool isVisible = (visId == _id);
	...
	vec3 delta = _shooterPower * _reflectance * fij;
